...
export const t7: PerformanceTask = {
  ...
  modules: ['m8', 'm9'],
  safety: [
    'The supply capacitors on either kind of board hold their charge after the mains is removed, and on a control board what they hold is rectified mains. Discharge them through a bleeder resistor and confirm with a meter that they read close to zero volts before touching the board.',
    'Some of the checks below need the unit powered with the case open, and you cannot tell by looking which points are safe to probe while it is. The mains half of a control board reaches parts that look like ordinary low voltage components, and some boards have no isolated half at all. So before you power it with the case open, go over the board with your teacher and have them mark the points that are safe to probe live, and the one point you may clip your earth lead to. Every point they have not marked is mains until they say otherwise, including the inlet, the fuse and anything you are unsure of.',
  ...
  steps: [
    ...
    'Unplug the unit and open the case. The first thing you touch inside is the bleeder resistor across the supply capacitors: discharge them and confirm they read close to zero volts before anything else, because on a control board that charge is rectified mains.',
    ...
    'If you took the previous step, then still powered and still only at the marked points, compare the signal at the input and at the output of the affected stage, working along the chain rather than at random.',
    'Unplug and discharge again before you unsolder or fit anything, and give the output devices and their heatsinks time to cool. They have been dissipating through the last two steps and an iron tip is not the only thing on that board that will burn you.',
  ...
  rubric: [
    ...
    { criterion: 'Safe working', descriptor: 'The safe points were agreed with the teacher before the board was powered with the case open and nothing outside them was probed live, the capacitors were proved discharged before contact, and the volume was down at switch on.', points: 6 },
  ],
}
