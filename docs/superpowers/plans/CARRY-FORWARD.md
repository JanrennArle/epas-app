...
- **`/labs` and `/tools` are still advertised and still dead.** `src/ui/Shell.tsx` links Labs, and the spec's route list also has `/tools` and `/settings`. Labs is the next plan; `/tools` and `/settings` have never been scheduled and should be either built or removed from the spec rather than left as permanent promises.
