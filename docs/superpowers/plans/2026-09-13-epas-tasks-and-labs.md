...
export const t1: PerformanceTask = {
  ...
  steps: [
    ...
    'Unplug the supply, discharge the filter capacitor through a bleeder resistor, and confirm with the meter that it reads close to zero volts.',
  ...
  rubric: [
    ...
    { criterion: 'Correct output', descriptor: 'The output voltage and its polarity match the schematic, within the regulator tolerance.', points: 5 },
...
export const t3: PerformanceTask = {
  ...
  steps: [
    ...
    'With the appliance still unplugged, check continuity through the supply cord from the plug pins to the internal terminals.',
    ...
    'Unplug it again before you close any panel you opened to reach the last fault.',
  ],
  rubric: [
...
export const t7: PerformanceTask = {
  ...
  safety: [
    ...
    'Three of the checks below need the amplifier powered. Clip your probes on before you switch on, keep one hand away from the chassis, and switch off again before you move them.',
  ...
  steps: [
    'Confirm the fault by ear first, and note whether it affects one channel or both, because that alone halves the search.',
    ...
    'Fit the replacement, reassemble, then restore power and compare both channels by ear at a low volume before raising it.',
  ],
  rubric: [
